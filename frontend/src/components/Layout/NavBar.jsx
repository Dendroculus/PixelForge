...
const NavDropdown = ({ title, children }) => (
  <div className="relative group">
    <button className="flex items-center gap-1.5 text-sm font-semibold text-slate-700 hover:text-indigo-600 transition-colors py-2">
      {title}
      <svg className="w-3.5 h-3.5 text-slate-400 group-hover:text-indigo-500 group-hover:rotate-180 transition-all duration-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <nav className="w-full border-b border-white/40 bg-white/40 backdrop-blur-xl sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-6">
          ...
          <a href="https://github.com/Dendroculus/image-upscaler-website" target="_blank" rel="noopener noreferrer" className="text-sm hover:text-slate-900 transition-colors flex items-center gap-1.5 font-bold text-slate-600">
          ...
          <div className="flex flex-col gap-1 text-sm font-medium text-slate-700">
            {Object.values(NavLinks).map((category) => (
              <div key={category.title}>
                ...
                {category.items.map((item) => {
                  const activeBg = item.isAi ? 'active:bg-indigo-50' : 'active:bg-slate-100';
                  return (
                    <Link 
                      key={item.id}
                      to={item.to} 
