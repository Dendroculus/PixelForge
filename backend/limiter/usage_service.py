...
from core.config import DatabaseConfig as DC
...
class UsageService:
    """Business logic for usage tracking, limits, and cleanup."""
    ...
    @classmethod
    async def check_daily_limit(cls, ip_address: str, limit_24h: int, feature: str) -> bool:
        if limit_24h <= 0:
            return False
        ...
        feature_ip = cls._get_feature_key(ip_address, feature)
        try:
            async with pool.acquire() as conn:
                used = await UsageRepository.get_usage_sum(conn, feature_ip)
    ...
    @classmethod
    async def increment_daily_limit(cls, ip_address: str, feature: str) -> None:
        pool = get_db_pool()
        if pool is None:
            return
        ...
        try:
            async with pool.acquire() as conn:
                await UsageRepository.increment_usage(conn, feature_ip)
        except Exception:
            logger.exception("Database error during usage increment.")

    @classmethod
    async def decrement_daily_limit(cls, ip_address: str, feature: str) -> None:
        pool = get_db_pool()
        if pool is None:
            return
        ...
        try:
            async with pool.acquire() as conn:
                await UsageRepository.decrement_usage(conn, feature_ip)
        except Exception:
            logger.exception("Database error during usage refund.")

    @classmethod
    async def get_usage_status(cls, ip_address: str, limit_24h: int, feature: str) -> dict:
        if limit_24h <= 0:
        ...
        try:
            async with pool.acquire() as conn:
                rec = await UsageRepository.get_usage_and_oldest_bucket(conn, feature_ip)
    ...
    async def run_database_cleanup(cls) -> int:
        """Removes expired usage records based on retention policy."""
        pool = get_db_pool()
        if pool is None:
        ...
        try:
            async with pool.acquire() as conn:
                deleted = await UsageRepository.delete_expired_records(conn, DC.USAGE_RETENTION_HOURS)
