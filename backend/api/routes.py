...
async def process_image_task(job_id: str, safe_filename: str, model_type: str, scale: int, client_ip: str) -> None:
    ...
    try:
        ...
        if success:
            ...
            await increment_daily_limit(client_ip)
        else:
...
@router.post(
    ...
    summary="Initialize direct-to-cloud upload",
)
@limiter.limit(LC.UPLOAD_RATE_LIMIT)
async def init_upload(request: Request, payload: InitRequest):
    ...
    # Just check, don't deduct yet
    is_allowed = await check_daily_limit(client_ip, limit_24h=LC.DAILY_USAGE_LIMIT)
...
@router.post(
    ...
    summary="Trigger AI processing after successful cloud upload",
)
@limiter.limit(LC.UPLOAD_RATE_LIMIT)
async def start_processing(
    ...
    if payload.job_id in _active_jobs:
        raise HTTPException(
    ...
    _active_jobs.add(payload.job_id)
    model_type = "general"
    ...
    return {"message": "Processing started", "job_id": payload.job_id}
